let ytPreloader = (function () {
	function ytPreloader () {
		let s = this;
		LExtends(s, LSprite, []);

		let backgroundBmp = new LBitmap(dataList["preloader_background"]);
		backgroundBmp.x = -130;
		backgroundBmp.y = -100;
		s.addChild(backgroundBmp);

		let m = 40;
		let progressBackgroundBmp = new LBitmap(dataList["preloader_bar_background"]);
