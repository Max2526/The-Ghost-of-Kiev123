let ytCarLayer = (function () {
	function ytCarLayer (carIndex) {
		let s = this;
		LExtends(s, LSprite, []);

		s.frameIndex = 0;
		s.frameMaxIndex = 70;

		s.carBmpd = dataList["cars_atlas"];

		s.indexToBmpdPropertiesList = null;

		s.car = null;

		s.initIndexToBmpdPropertiesList();

		s.addCar("player", carIndex, 0);
	}

	ytCarLayer.prototype.initIndexToBmpdPropertiesList = function () {
		let s = this, w = s.carBmpd.width, h = s.carBmpd.height;

		let l = LGlobal.divideCoordinate(w, h, 3, 5);

		s.indexToBmpdPropertiesList = {
            // гравець //
			"player" : [
				l[0][0],
				l[1][0],    
				l[1][3],
				l[0][3]
			],
            // спавн перешкод //
			"obstacle" : [
				l[1][1],
				l[0][2],
				l[0][4],
				l[1][1],
				l[1][2],
				l[1][4],
				l[2][0],
				l[2][2],
				l[2][3],
				l[2][4]
			]
		};
	};

	ytCarLayer.prototype.addCar = function(group, index, pi, speed) {
		let s = this;

		let data = s.indexToBmpdPropertiesList[group][index];
		let car = new ytCar(s.carBmpd, data, pi);
		s.addChild(car);

		if (group == "player" && !s.car) {
			s.car = car;
			s.car.y = LGlobal.height - s.car.getHeight() - 20;
			s.car.addShape(LShape.RECT, [0, 18, s.car.getWidth(), 80]);
		} else {
			car.y = -200;
			car.speed = speed;

			if (index != 0 && index != 4) {
				car.addShape(LShape.RECT, [0, 18, s.car.getWidth(), 80]);
			}else if (index == 4) {
				car.addShape(LShape.RECT, [0, 0, s.car.getWidth(), 115]);
			}
		}
	};

	ytCarLayer.prototype.loop = function () {
		let s = this;

		s.changeObstacleCarPosition();

		s.addObstacleCar();

		return s.checkHit();
	};

	ytCarLayer.prototype.changeObstacleCarPosition = function () {
		let s = this, rml = new Array();

		for (let k = 1, l = s.numChildren; k < l; k++) {
			let o = s.getChildAt(k);

			if (o) {
				o.y += o.speed;

				if (Math.random() < 0.00000002) {  // переміщення перешкод (швидкість) //
					if (!o.changeDirTween) {
						let toPosition = Math.round(Math.random()),
						cDir = o.dir,
						canMove = true,
						oSh = o.shapes.slice(0);

						o.x = o.roadPositionList[toPosition];

						o.clearShape();

						for (let j = 0; j < l; j++) {
							let oO = s.getChildAt(j), oOSh = oO.shapes.slice(0);

							if (oO.objectIndex == o.objectIndex) {
								continue;
							}

							oO.clearShape();

							if (j == 0) {
								let ox = oy = 60,
								ow = oO.getWidth() + ox * 2, 
								oh = oO.getHeight() + oy * 2;

								oO.addShape(LShape.RECT, [-ox, -oy, ow, oh]);
							}

							let hr = o.hitTestObject(oO);

							oO.shapes = oOSh;

							if (hr) {
								canMove = false;

								break;
							}
						}

						o.shapes = oSh;

						o.x = o.roadPositionList[cDir];

						if (canMove) {
							o.moveTo(toPosition, 1);
						}
					}
				}

				if (o.y >= LGlobal.height) {
					rml.push(o);
				}
			}
		}

		for (let i = 0, l = rml.length; i < l; i++) {
			rml[i].destroy();
		}
	};

	ytCarLayer.prototype.addObstacleCar = function () {
		let s = this;

		if (s.frameIndex++ > s.frameMaxIndex) {
			let l = s.indexToBmpdPropertiesList["obstacle"],
			index = Math.round(Math.random() * (l.length - 1)),
			pi = Math.round(Math.random()),
			speed = 5 + Math.floor(Math.random() * 3);

			s.addCar("obstacle", index, pi, speed);

			s.frameIndex = 0;
		}
	};

	ytCarLayer.prototype.checkHit = function () {
		let s = this;

		for (let k = 1, l = s.numChildren; k < l; k++) {
			let oA = s.getChildAt(k), shA = oA.shapes.slice(0);

			oA.clearShape();

			for (let i = 1; i < l; i++) {
				let oB = s.getChildAt(i), shB = oB.shapes.slice(0);

				if (oB.objectIndex == oA.objectIndex) {
					continue;
				}

				oB.clearShape();

				if (oB.speed != oA.speed && oA.hitTestObject(oB)) {
					if (oB.speed > oA.speed) {
						oB.speed = oA.speed;
					} else {
						oA.speed = oB.speed;
					}
				}

				oB.shapes = shB;
			}

			oA.shapes = shA;
		}

		if (s.car) {
			for (let j = 1, l = s.numChildren; j < l; j++) {
				let oC = s.getChildAt(j);

				if (s.car.hitTestObject(oC)) {
					let cx = s.car.x,
					cy = s.car.y,
					cw = s.car.getWidth(),
					ch = s.car.getHeight();

					s.car.destroy();

					return [true, cx, cy, cw, ch];
				}
			}
		}

		return [false];
	};

	return ytCarLayer;
})();