let ytStreetView = (function () {
	...
	ytStreetView.prototype.addStartStreetView = function () {
		...
		let bmpd = dataList["misc_atlas"].clone();
		bmpd.setProperties(0, 0, 520, 130);
		let bmp = new LBitmap(bmpd);
	...
	ytStreetView.prototype.addStreetViewAsPlace = function () {
		let s = this, streetViewList = ytStreetView.StreetViewList[s.place];

		if (streetViewList) {
			let item = streetViewList[Math.round(Math.random() * (streetViewList.length - 1))];
	...
	ytStreetView.prototype.loop = function () {
		...
		for (let i = 0, l = s.streetViewLayer.numChildren; i < l; i++) {
			let o = s.streetViewLayer.getChildAt(i);
			o.y += s.speed;
