let ytGameLayer = (function () {
	function ytGameLayer (car, place) {
		...
		s.point = new ytPoint();
		s.point.x = s.point.y = 10; // Розміщення кількості метрів //
	...
	ytGameLayer.prototype.addPauseBtn = function () {
		...
		let d = list[0][0];
		let normalBmpd = bmpd.clone();
		normalBmpd.setProperties(d.x, d.y, d.width, d.height);

		d = list[1][0];
		let overBmpd = bmpd.clone();
		overBmpd.setProperties(d.x, d.y, d.width, d.height);

		d = list[2][0];
		let downBmpd = bmpd.clone();
		downBmpd.setProperties(d.x, d.y, d.width, d.height);

		let normalBmp = new LBitmap(normalBmpd);
		let overBmp = new LBitmap(overBmpd);
		let downBmp = new LBitmap(downBmpd);
		...
		s.pauseBtn.addEventListener(LMouseEvent.MOUSE_UP, function () {
			...
			if (s.carLayer) {
				for (let i = 0, l = s.carLayer.numChildren; i < l; i++) {
					let o = s.carLayer.getChildAt(i), t = o.changeDirTween;
		...
		if (s.carLayer) {
			...
			if (s.gameOver) {
				...
				explosion.addEventListener(ytExplosion.EVENT_PLAY_OVER, function () {
					let resultBox = new ytResultBox(s.point.num);
